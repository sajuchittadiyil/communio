...
Deno.test("Wave 2 effective-year overlap includes boundary and open terms", () => {
  assert(
    overlapsYear({ from_date: "2015-12-31", to_date: null }, 2015),
    "Dec 31 start excluded",
  );
  assert(
    overlapsYear({ from_date: "2010-01-01", to_date: "2015-01-01" }, 2015),
    "Jan 1 end excluded",
  );
  assert(
    !overlapsYear({ from_date: "2016-01-01", to_date: null }, 2015),
    "future term included",
  );
});
...
Deno.test("Wave 2 people lists deduplicate overlapping assignment rows", () => {
  assert(
    ...
    "members were not deduplicated",
  );
});

Deno.test("Wave 2 tenure uses inclusive completed dates without rounding partial terms", () => {
  const first = durationLabel({
    from_date: "2018-07-01",
    to_date: "2024-06-30",
  });
  const threeYears = durationLabel({
    from_date: "2020-07-01",
    to_date: "2023-06-30",
  });
  const partial = durationLabel({
    from_date: "2024-07-01",
    to_date: "2026-08-07",
  });
  const second = durationLabel(
...
Deno.test("Wave 2 historical superior zero results remain domain-specific", () => {
  ...
  assert(
    message ===
      "I found no recorded Community Superior appointment for St. Antony Community, Kolkata covering 2015.",
    "superior zero-result wording fell through to membership",
  );
  assert(
    !message.includes("recorded members"),
    "superior zero result mentions members",
  );
});
