...
function contextFor(row: CompetencyRow): AskCommunioContext | undefined {
  ...
  if (row.conversation_id === "A" || row.conversation_id === "F") {
    return {
      ...
      focus_entity_id: "benchmark-community",
      focus_entity_name: "Benchmark Community",
    };
  }
  return {
    focus_entity_type: "member",
    focus_entity_id: "benchmark-member",
    focus_entity_name: "Benchmark Member",
    last_answer_entity_type: "member",
    last_answer_entity_name: "Benchmark Member",
  };
}
...
function failuresFor(row: CompetencyRow): string[] {
  ...
  if (actual.intent !== row.expected_intent) {
    failures.push(`intent ${actual.intent} != ${row.expected_intent}`);
  }
  if (row.expected_output_type === "count" && actual.outputType !== "count") {
    failures.push("count output not detected");
  }
  if (row.expected_time_scope) {
    const [from, to] = row.expected_time_scope.split("..").map(Number);
    if (actual.year !== from) {
      failures.push(`year ${actual.year ?? "none"} != ${from}`);
    }
    if (to && actual.yearTo !== to) {
      failures.push(`end year ${actual.yearTo ?? "none"} != ${to}`);
    }
  }
  return failures;
}
...
Deno.test("Ask Communio competency suite protects P0 parser and context regressions", () => {
  const suiteUrl = new URL(
    "../../../docs/ask_communio_competency_suite.csv",
    import.meta.url,
  );
  ...
  const p0Regressions = failures.filter(({ row }) =>
    row.priority === "P0" && row.expected_behavior !== "KNOWN_GAP"
  );
  const knownGaps =
    rows.filter((row) => row.expected_behavior === "KNOWN_GAP").length;
  console.log(
    `ASK COMMUNIO COMPETENCY REPORT: total=${rows.length}, parser_failures=${failures.length}, known_gaps=${knownGaps}, p0_regressions=${p0Regressions.length}`,
  );
  assert(
    rows.length >= 250 && rows.length <= 300,
    `expected 250–300 cases, found ${rows.length}`,
  );
  assert(
    p0Regressions.length === 0,
    p0Regressions.slice(0, 20).map(({ row, reasons }) =>
      `${row.id} ${row.question}: ${reasons.join("; ")}`
    ).join("\n"),
  );
});
...
Deno.test("competency conversations cover focus, ambiguity, and unsupported-turn preservation", () => {
  ...
  const rows = parseCsv(Deno.readTextFileSync(suiteUrl)).filter((row) =>
    row.conversation_id
  );
  assert(
    new Set(rows.map((row) => row.conversation_id)).size === 6,
    "expected conversations A–F",
  );
  for (const row of rows) {
    const reasons = failuresFor(row);
    assert(
      reasons.length === 0,
      `${row.id} conversation ${row.conversation_id}.${row.turn_number}: ${
        reasons.join("; ")
      }`,
    );
  }
