...
export function overlapsYear(row: Wave2Row, year: number): boolean {
  ...
  return (!start || start <= `${year}-12-31`) &&
    (!end || end >= `${year}-01-01`);
}

export function startsOrEndsInYear(
  row: Wave2Row,
  year: number,
  boundary: "started" | "ended",
): boolean {
  const date = boundary === "started"
    ? value(row, "from_date", "start_date")
    : value(row, "to_date", "end_date");
...
export function durationLabel(row: Wave2Row, through = new Date()): string {
  ...
  if (days < 0) {
    months -= 1;
    days += new Date(Date.UTC(end.getUTCFullYear(), end.getUTCMonth(), 0))
      .getUTCDate();
  }
  if (months < 0) {
    years -= 1;
  ...
  ].filter(Boolean).join(" ") || "less than one day";
}

export function historicalAssignmentEmptyMessage(
  kind: "community" | "ministry",
  name: string,
  year: number,
  superiorOnly = false,
): string {
  if (kind === "community" && superiorOnly) {
    return `I found no recorded Community Superior appointment for ${name} covering ${year}.`;
  }
