-- Extend the caller-bound self profile with only the signed-in Member's own
-- approved contact fields, sourced from Communio's canonical contact view.
...
as $$
  select jsonb_build_object(
    ...
    'mobile', self_contact.mobile,
    'whatsapp', self_contact.whatsapp,
    'official_email', self_contact.official_email,
  ...
  left join public.v_demo_member_public_contacts self_contact
    on self_contact.member_id = member.id
  left join lateral (
  ...
  left join lateral (
    ...
    ) order by event.event_date) as events
...
-- Directory-safe current residents. Assignment dates are used only by the
-- predicate and are not exposed by the view.
create or replace view public.v_member_community_residents_safe
...
select
  ...
  directory.ecclesiastical_title_code,
  directory.photo_url,
  directory.member_status_code,
  assignment.responsibility_code as community_responsibility_code,
  directory.ministry_name,
  directory.ministry_responsibility_code
from public.member_community_assignments assignment
join public.v_member_directory_safe directory
  on directory.member_id = assignment.member_id
where assignment.from_date <= current_date
  and (assignment.to_date is null or assignment.to_date >= current_date)
  and public.current_access_role() in ('member', 'provincial');

revoke all on public.v_member_community_residents_safe from public, anon;
grant select on public.v_member_community_residents_safe to authenticated;

comment on view public.v_member_community_residents_safe is
  'Current community residents with directory-safe identity and assignment context only.';
